...
const Dashboard: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      vacantUnits,
      monthlyRentExpected,
      monthlyRentCollected: collectedThisMonth,
    ...
    filteredPayments.forEach((p: Payment) => {
      ...
      if (p.type === 'RENT') {
        ...
        if (p.isRefunded) {
          ...
          // Refunds reduce the aggregate totals in the distribution list
          byMode[mode] = (byMode[mode] || 0) - p.amount;
          byRecipient[recipient] = (byRecipient[recipient] || 0) - p.amount;
        } else {
  ...
  const navigateYear = (dir: number) => {
    setSelectedYear((parseInt(selectedYear) + dir).toString());
  };
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 max-w-[1600px] mx-auto px-4 md:px-0">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-indigo-600 font-bold mb-1">
            ...
            <span className="text-[10px] uppercase tracking-widest font-black">System Terminal Active</span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'MTD Revenue', val: `$${stats.monthlyRentCollected.toLocaleString()}`, sub: 'Settled rent', icon: DollarSign, color: 'bg-indigo-600' },
          { label: 'Occupancy', val: `${Math.round(stats.occupancyRate)}%`, sub: `${stats.activeUnits} Units Live`, icon: Home, color: 'bg-emerald-600' },
          { label: 'Target Yield', val: `$${stats.monthlyRentExpected.toLocaleString()}`, sub: 'Portfolio cap', icon: Target, color: 'bg-slate-900' },
          { label: 'Velocity', val: `${Math.round(stats.collectionRate)}%`, sub: 'Collection speed', icon: TrendingUp, color: 'bg-amber-500' },
        ].map((item, i) => (
        ...
        <div className="lg:col-span-8 bg-white p-8 md:p-12 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col min-h-[550px]">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-12 gap-6">
            <div>
              ...
              <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">Target vs Actual Liquidity</p>
            </div>
            <div className="flex gap-6">
               <div className="flex items-center gap-2 text-[10px] font-black uppercase"><div className="w-3 h-3 rounded-full bg-slate-100 border border-slate-200"></div> Target</div>
               <div className="flex items-center gap-2 text-[10px] font-black uppercase"><div className="w-3 h-3 rounded-full bg-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.5)]"></div> Collected</div>
            </div>
          </div>
          <div className="flex-1 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={propertyChartData} margin={{ top: 0, right: 10, left: -10, bottom: 0 }} barGap={12}>
                ...
                <Tooltip 
                  cursor={{ fill: '#f8fafc' }}
                  contentStyle={{ backgroundColor: '#0f172a', border: 'none', borderRadius: '1.25rem', boxShadow: '0 20px 50px rgba(0,0,0,0.2)' }}
        ...
        <div className="lg:col-span-4 bg-white p-8 md:p-12 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col">
          ...
          <div className="flex-1 space-y-4">
            {stats.overdueUnitsList.length > 0 ? stats.overdueUnitsList.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between p-5 bg-slate-50 rounded-[2rem] border border-transparent hover:border-red-100 hover:bg-white transition-all group shadow-sm hover:shadow-xl">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-slate-400 group-hover:bg-red-500 group-hover:text-white shadow-sm transition-all">
              ...
              <div className="h-full flex flex-col items-center justify-center text-center opacity-50 py-12">
                ...
                <p className="text-[10px] font-bold text-slate-400 mt-2">No critical overdue detected.</p>
              </div>
            )}
          </div>
          ...
          <button 
            ...
            className="w-full mt-8 py-5 bg-slate-950 text-white rounded-[1.5rem] font-black uppercase text-[11px] tracking-[0.2em] shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3 group"
          >
            Open Global Ledger <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </button>
        </div>
      </div>

      {/* Financial Distribution Section */}
      <div className="bg-slate-950 rounded-[4rem] p-10 lg:p-16 text-white relative overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.3)] border border-white/5">
         ...
         <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-emerald-500/5 blur-[120px] rounded-full -translate-x-1/2 translate-y-1/2 pointer-events-none"></div>
         ...
         <div className="relative z-10 grid grid-cols-1 xl:grid-cols-12 gap-12 xl:gap-20">
            <div className="xl:col-span-4 flex flex-col justify-center">
               <div className="inline-flex items-center gap-3 bg-white/5 px-4 py-2 rounded-full border border-white/10 text-indigo-400 font-black uppercase tracking-[0.3em] text-[10px] mb-8">
                  <BarChart3 className="w-4 h-4" /> Settlement Intel
               </div>
               <h2 className="text-5xl font-black tracking-tighter leading-none mb-6 uppercase">Capital <br /> <span className="text-indigo-500">Distribution</span></h2>
               <p className="text-slate-400 text-lg font-medium leading-relaxed mb-12">Detailed audit of net inflow including collected rent, security deposits, and processed refunds.</p>
               ...
               <div className="space-y-4">
                  <div className="bg-white/[0.03] p-6 rounded-[2.5rem] border border-white/5 flex items-center justify-between group hover:bg-white/[0.05] transition-all">
                     <div className="flex items-center gap-5">
                        ...
                        <div>
                           <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Rent Inflow</p>
                  ...
                  <div className="bg-white/[0.03] p-6 rounded-[2.5rem] border border-white/5 flex items-center justify-between group hover:bg-white/[0.05] transition-all">
                     <div className="flex items-center gap-5">
                        ...
                        <div>
                           <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Deposit Asset</p>
                  ...
                  <div className="bg-white/[0.03] p-6 rounded-[2.5rem] border border-white/5 flex items-center justify-between group hover:bg-white/[0.05] transition-all">
                     <div className="flex items-center gap-5">
                        ...
                        <div>
                           <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Refunds Deducted</p>
            ...
            <div className="xl:col-span-8 bg-white/[0.02] rounded-[3.5rem] border border-white/5 p-8 lg:p-12 backdrop-blur-md flex flex-col min-h-[700px]">
               {/* Fixed Layout: Filter Type -> Date Selector -> Tabs Below */}
               <div className="flex flex-col gap-10 mb-16">
                  {/* Row 1: Filter Type Selector */}
                  <div className="flex justify-center xl:justify-start">
                    <div className="flex items-center gap-2 bg-slate-900/90 p-1.5 rounded-2xl border border-white/10 shadow-2xl">
                      {['monthly', 'annual', 'custom'].map(t => (
                          <button 
                            key={t}
                            onClick={() => setFilterType(t as FilterType)}
                            className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filterType === t ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/30' : 'text-slate-500 hover:text-white'}`}
                          >
                            {t}
                          </button>
                      ))}
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-6 px-2">
                     <div className="flex items-center gap-6 bg-white/10 px-6 py-4 rounded-2xl border border-white/10 w-full">
                        {filterType === 'monthly' && (
                        ...
                        {filterType === 'annual' && (
                          <div className="flex items-center justify-between w-full gap-8">
                             <button onClick={() => navigateYear(-1)} className="p-2 hover:bg-white/10 rounded-xl transition-colors"><ChevronLeft className="w-5 h-5" /></button>
                             <div className="flex items-center gap-4">
                               <TrendingUp className="w-5 h-5 text-indigo-400" />
                               <span className="text-sm font-black text-white tracking-widest">{selectedYear} Portfolio Year</span>
                             </div>
                             <button onClick={() => navigateYear(1)} className="p-2 hover:bg-white/10 rounded-xl transition-colors"><ChevronRight className="w-5 h-5" /></button>
                          </div>
                        )}
                        {filterType === 'custom' && (
                          <div className="flex items-center justify-center gap-6 w-full">
                             <input type="date" className="bg-transparent border-none text-[10px] font-black text-white outline-none [color-scheme:dark] uppercase" value={startDate} onChange={e => setStartDate(e.target.value)} />
                             <ArrowRightLeft className="w-4 h-4 text-slate-700" />
                             <input type="date" className="bg-transparent border-none text-[10px] font-black text-white outline-none [color-scheme:dark] uppercase" value={endDate} onChange={e => setEndDate(e.target.value)} />
                          </div>
                        )}
                     </div>
                  </div>

                  {/* Row 3: Tabs Toggle Below Date Navigation */}
                  <div className="flex justify-center xl:justify-start">
                     <div className="flex items-center gap-2 bg-slate-900/90 p-1.5 rounded-2xl border border-white/10 shadow-2xl w-full sm:w-auto">
                        <button 
                           onClick={() => setReportView('mode')}
                           className={`flex-1 sm:flex-none flex items-center justify-center gap-3 px-10 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${reportView === 'mode' ? 'bg-emerald-600 text-white shadow-xl shadow-emerald-600/30' : 'text-slate-500 hover:text-white'}`}
                        >
                           <CreditCard className="w-4 h-4" /> Transfer Modes
                        </button>
                        <button 
                           onClick={() => setReportView('recipient')}
                           className={`flex-1 sm:flex-none flex items-center justify-center gap-3 px-10 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${reportView === 'recipient' ? 'bg-emerald-600 text-white shadow-xl shadow-emerald-600/30' : 'text-slate-500 hover:text-white'}`}
                        >
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 xl:gap-24 items-center flex-1">
                     ...
                     {reportData.pieData.length > 0 ? (
                        <div className="h-full w-full relative">
                           <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
                              <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] mb-1">Net Revenue</p>
                           ...
                           <ResponsiveContainer width="100%" height="100%">
                              <PieChart>
                                 ...
                                 <Tooltip contentStyle={{ backgroundColor: '#0f172a', border: 'none', borderRadius: '1.25rem', color: '#fff', fontSize: '11px', fontWeight: 'bold', padding: '16px' }} />
                              </PieChart>
                           </ResponsiveContainer>
                        </div>
                     ) : (
                        <div className="h-full flex flex-col items-center justify-center text-slate-700 border-2 border-dashed border-white/5 rounded-full">
                           ...
                           <p className="text-[11px] font-black uppercase tracking-[0.2em] opacity-40">No transactional volume found</p>
                        </div>
                     )}
                  </div>
                  ...
                  <div className="flex flex-col gap-4 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
                     {reportData.pieData.map((item, i) => (
                        <div key={item.name} className="flex justify-between items-center group bg-white/[0.02] p-6 rounded-3xl border border-white/5 hover:border-white/10 hover:bg-white/[0.04] transition-all">
                           <div className="flex items-center gap-5">
                              <div className="w-3.5 h-3.5 rounded-full ring-4 ring-white/5 flex-shrink-0" style={{ backgroundColor: COLORS[i % COLORS.length] }}></div>
                              <div>
                                 <span className="text-base font-black text-slate-200 group-hover:text-white transition-colors uppercase tracking-tight block">{item.name}</span>
                           ...
                           <div className="text-right flex-shrink-0">
                              ...
                              <p className="text-[9px] font-bold text-indigo-400 uppercase tracking-tighter mt-1">Verified Inflow</p>
                           </div>
                        </div>
                     ))}
                  </div>
               ...
               <div className="mt-16 pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
                  <div className="flex items-center gap-3">
                     ...
                     <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em]">Encrypted Audit Trail Active • Ledger V1.0</p>
                  </div>
                  <button 
                    onClick={() => navigate('/collection')}
                    className="flex items-center gap-6 group px-8 py-4 bg-white/5 hover:bg-white/10 rounded-[1.5rem] border border-white/5 transition-all w-full md:w-auto"
                  >
                     <div className="text-left">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block group-hover:text-indigo-400 transition-colors">Period Total (Rent + Deposits - Refunds)</span>
                        <p className="text-3xl font-black text-white tracking-tighter group-hover:scale-105 transition-transform origin-left">${reportData.total.toLocaleString()}</p>
                     </div>
